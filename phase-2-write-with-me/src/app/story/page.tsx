...
export default async function StoryPage({ searchParams }: { searchParams: Promise<{ id?: string; title?: string; text?: string; child?: string; idea?: string; length?: string; tone?: string; guestName?: string }> }) {
  const { id, title, text, child, idea, length, tone, guestName } = await searchParams;
